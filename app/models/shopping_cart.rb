class ShoppingCart < ApplicationRecord
    ...
    scope :bought_days_sqlite, -> { bought_carts.order(updated_at: :desc).group("strftime('%Y-%m', updated_at, '+09:00')").pluck(:updated_at) }
    scope :bought_month_mysql, -> { bought_carts.order(updated_at: :desc).group("date_format(updated_at + interval 9 hour,'%Y-%m')").pluck(:updated_at) }
    scope :bought_days_mysql, -> { bought_carts.order(updated_at: :desc).group("date_format(updated_at + interval 9 hour,'%Y-%m')").pluck(:updated_at) }
